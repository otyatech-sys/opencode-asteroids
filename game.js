...
const TRIPLE_DURATION     = 5;      // segundos de duración
const TRIPLE_SPAWN_CHANCE = 0.20;   // probabilidad al destruir asteroide
const TRIPLE_TTL          = 8;      // segundos antes de desaparecer
const TRIPLE_SPREAD       = 0.21;   // ~12° de separación
...
const SHIELD_DURATION    = 7;     // segundos de duración
const SHIELD_SPAWN_CHANCE = 0.18; // probabilidad al destruir asteroide
const SHIELD_TTL         = 10;    // segundos antes de desaparecer si no se recoge
const SHIELD_MAX         = 1;     // máximo simultáneos en pantalla
const SHIELD_RADIUS      = 12;    // radio del power-up visual
const SHIELD_COLOR       = [0, 255, 180]; // color verde/celeste del escudo
...
class TripleShot {
  ...
  draw() {
    ...
    ctx.fillStyle = `rgba(255, 160, 0, ${0.22 * alpha})`;
    ctx.beginPath();
    ctx.arc(0, 0, this.radius + 5, 0, Math.PI * 2);
    ctx.fill();
...
class Shield {
  ...
  draw() {
    ...
    // Brillo exterior
    ctx.fillStyle = `rgba(${SHIELD_COLOR[0]}, ${SHIELD_COLOR[1]}, ${SHIELD_COLOR[2]}, ${0.22 * alpha})`;
    ctx.beginPath();
    ctx.arc(0, 0, this.radius + 5, 0, Math.PI * 2);
    ctx.fill();

    // Círculo principal
    ctx.strokeStyle = `rgba(${SHIELD_COLOR[0]}, ${SHIELD_COLOR[1]}, ${SHIELD_COLOR[2]}, ${alpha})`;
    ctx.lineWidth   = 2;
    ctx.beginPath();
    ctx.arc(0, 0, this.radius, 0, Math.PI * 2);
    ctx.stroke();

    // Icono de escudo (rombo)
    ctx.strokeStyle = `rgba(${SHIELD_COLOR[0]}, ${SHIELD_COLOR[1]}, ${SHIELD_COLOR[2]}, ${alpha})`;
    ctx.lineWidth   = 1.5;
    ctx.beginPath();
    ctx.moveTo(0, -6);
...
class Ship {
  ...
  draw() {
    ...
    // Escudo protector
    if (this.shieldTimer > 0) {
      const alpha = this.shieldTimer < 2 ? 0.3 + Math.sin(Date.now() * 0.01) * 0.2 : 0.6;
      ctx.strokeStyle = `rgba(${SHIELD_COLOR[0]}, ${SHIELD_COLOR[1]}, ${SHIELD_COLOR[2]}, ${alpha})`;
      ctx.lineWidth = 2;
      ctx.beginPath();
      ...
      // Brillo interior
      ctx.fillStyle = `rgba(${SHIELD_COLOR[0]}, ${SHIELD_COLOR[1]}, ${SHIELD_COLOR[2]}, ${alpha * 0.15})`;
      ctx.fill();
    }
...
// ── Estado del juego ──────────────────────────────────────────────────────────
let ship, bullets, asteroids, particles, speedBoosts, shields, shootingStars, tripleShots;
...
function trySpawnTripleShot(x, y) {
  if (tripleShots.length > 0) return;
  if (Math.random() < TRIPLE_SPAWN_CHANCE)
    tripleShots.push(new TripleShot(x, y));
}

function trySpawnShield(x, y) {
  if (shields.length >= SHIELD_MAX) return;
...
function update(dt) {
  ...
  for (const t of tripleShots) {
    if (!t.dead && dist(ship, t) < ship.radius + t.radius) {
      t.dead = true;
      ship.tripleShotTimer = TRIPLE_DURATION;
      explode(t.x, t.y, 6);
    }
  }
  tripleShots = tripleShots.filter(t => !t.dead);
...
function drawHUD() {
  ...
  const bars = [];
  if (ship.tripleShotTimer > 0 && !ship.dead)
    bars.push({ t: ship.tripleShotTimer, d: TRIPLE_DURATION, c: [255, 180, 0] });
  if (ship.speedBoostTimer > 0 && !ship.dead)
    bars.push({ t: ship.speedBoostTimer, d: BOOST_DURATION, c: [0, 190, 255] });
  if (ship.shieldTimer > 0 && !ship.dead)
    bars.push({ t: ship.shieldTimer, d: SHIELD_DURATION, c: SHIELD_COLOR });
  ...
  bars.forEach((bar, i) => {
    ...
    const y = 34 + i * 9;
    const fill = (bar.t / bar.d) * barWidth;

    ctx.strokeStyle = `rgba(${bar.c[0]}, ${bar.c[1]}, ${bar.c[2]}, 0.9)`;
    ctx.strokeRect(x, y, barWidth, barHeight);
    ctx.fillStyle = `rgba(${bar.c[0]}, ${bar.c[1]}, ${bar.c[2]}, 0.75)`;
